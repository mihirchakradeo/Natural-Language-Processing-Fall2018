...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  data_index1 = data_index%len(data)
  
  start = max(data_index1 - skip_window, 0)
  end = min(data_index1 + skip_window, len(data))
  c = 0

  # iterating over the given data
  while c < batch_size:
    if end == len(data) or c == batch_size:
      break
    
    # creating word window in consideration
    word_window = data[data_index1:data_index1+window_size]

    # center word
    label = len(word_window)//2
    
    # gather left
    for left in range(label-1, label-skip_window-1, -1):
      batch[c] = word_window[label]
      labels[c][0] = word_window[left]
      c += 1
    ...
    #gather right
    for right in range(label+1, label+skip_window+1):
      batch[c] = word_window[label]
      labels[c][0] = word_window[right]
      c += 1
